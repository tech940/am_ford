...
import { getVehicle, vehicles, dealerInfo, DELIVERY_CLAIM, type Vehicle } from "@/lib/vehicles";
  ...
  head: ({ loaderData }) => {
    ...
    const faqSchema = {
      ...
      mainEntity: [
        {
          "@type": "Question",
          name: `Is this ${v.year} ${v.make} ${v.model} ${v.trim} in stock at AM Ford in ${dealerInfo.city}?`,
          acceptedAnswer: {
            "@type": "Answer",
            text: `Yes, this ${v.year} ${v.make} ${v.model} ${v.trim} is currently in stock and available for immediate test drives at AM Ford, ${dealerInfo.address}, serving Ashtabula County and Northeast Ohio.`,
          },
        },
        {
          "@type": "Question",
          name: `What is the price and monthly payment for this ${v.year} ${v.model}?`,
          acceptedAnswer: {
            "@type": "Answer",
            text: `This ${v.year} ${v.model} ${v.trim} is listed at $${v.price.toLocaleString()}. Flexible auto loan terms with competitive APR rates are available with custom down payment options.`,
          },
        },
        {
          "@type": "Question",
          name: `Does this ${v.model} come with warranty coverage in Ohio?`,
          acceptedAnswer: {
            "@type": "Answer",
            text: `Yes, AM Ford provides warranty protection on eligible vehicles, backed by our certified Ford service technicians.`,
          },
        },
        {
          "@type": "Question",
          name: `Can this ${v.year} ${v.model} be delivered to my home?`,
          acceptedAnswer: {
            "@type": "Answer",
            text: `${DELIVERY_CLAIM} Ask our team at AM Ford in ${dealerInfo.locality} about delivery options for this ${v.year} ${v.model}.`,
          },
        },
      ],
    };
        ...
        <div className="mx-auto max-w-7xl px-6 space-y-16">
          ...
          <div className="max-w-4xl space-y-6">
            ...
            <div className="space-y-3">
              {[
                {
                  q: `Is this ${v.year} ${v.make} ${v.model} ${v.trim} currently available for a test drive in ${dealerInfo.city}?`,
                  a: `Yes, this vehicle is in stock at our AM Ford showroom at ${dealerInfo.address}, serving Ashtabula County drivers and Northeast Ohio. You can book a test drive online or call our team at ${dealerInfo.phone}.`,
                },
                {
                  q: `What financing options are available for this ${v.model}?`,
                  a: `AM Ford offers flexible auto financing terms through local Ohio credit unions and national lenders. You can estimate your monthly payment using our calculator or apply online for 60-second pre-approval.`,
                },
                {
                  q: `Can I trade in my vehicle toward this ${v.year} ${v.model}?`,
                  a: `Absolutely. We accept all vehicle makes and models as trade-ins, and we offer top-dollar valuations based on real-time market data.`,
                },
                {
                  q: `Can this ${v.year} ${v.model} be delivered to my home?`,
                  a: `${DELIVERY_CLAIM} Call ${dealerInfo.phone} and our team in ${dealerInfo.locality} will confirm delivery options for this ${v.model}.`,
                },
              ].map((faq, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="overflow-hidden rounded-2xl bg-card border border-border transition-all"
                  >
                    <button
                      onClick={() => setOpenFaq(isOpen ? null : idx)}
                      className="flex w-full items-center justify-between p-5 text-left font-semibold text-ink transition hover:text-primary"
                    >
                    ...
                    <AnimatePresence>
                      {isOpen && (
                        <motion.div
                          ...
                          className="border-t border-border/60 bg-surface/40 px-5 py-4 text-xs sm:text-sm text-muted-foreground leading-relaxed"
                        >
                          {faq.a}
                        </motion.div>
                      )}
